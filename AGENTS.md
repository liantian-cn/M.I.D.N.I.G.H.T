...
- 处理 12.1 游戏 API、secret values、Aura、FrameXML 变化时，先读 `Phantom/.context/README.md`。
- 官方 API 变化记录保存在 `Phantom/.context/api-changes/`；冲突时新版本优先，12.1.0 仍是 beta / PTR，依赖细节前提醒用户刷新。
- 处理 `Phantom/` 或 `Copilot/` 时，读对应目录内的 `AGENTS.md` 和 `.context/README.md`。
...
- 根目录不再保留 `.context/` 事实入口；未来 `Phantom/` 和 `Copilot/` 会作为独立仓库根。
- WoW 游戏 API / 12.1 迁移风险放在 `Phantom/.context/`。
- Python 外部程序侧资料放在 `Copilot/.context/`。
