...
- Keep the git branch on lowercase `my-feature`. If it is not `my-feature`, switch before editing.
...
- Use this only when the user explicitly asks for a plan or the task is too ambiguous to implement safely.
- Produce a concise implementation plan and wait for the user's approval before code work when approval is required.
- Do not simulate execution while planning.
...
When using these role files, read the prompt and either apply the role inline or delegate only when the user has explicitly asked for sub-agents or parallel agent work.
...
Use this skill directly when its frontmatter description matches the task or when the user explicitly asks for `$dejavu-coder`.
