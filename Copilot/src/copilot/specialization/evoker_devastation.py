# 摘要：湮灭唤魔师专精定义，迁移自 PhantomProject 上游 lua 并按本地容器约定重写。

# 描述：
# 同步上游 src/class/evoker_devastation.lua 的 SpellList、ChargeList、PartyBuff 字段。表 key 统一为
...
# spell_list：17 项；index 3/8 具备 charge。
# charge_list：黑曜鳞片、悬空两项，maxValue 均为 8（上游为 Phantom 0..8 fallback）。
# party_buff：青铜祝福单对象。
