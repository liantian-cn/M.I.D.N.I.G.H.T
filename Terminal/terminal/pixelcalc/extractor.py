...
def get_player_status(matrix: MatrixDecoder) -> dict[str, Any]:
    ...
    status["unitClass"] = COLOR_MAP["CLASS"].get(
        matrix.getCell(50, 14).color_string, "UNKNOWN"
    )  # 职业
    status["unitRole"] = COLOR_MAP["ROLE"].get(
        matrix.getCell(50, 15).color_string, "NONE"
    )  # 职责
    ...
    status["unitHasDispellableDebuff"] = matrix.getCell(
        53, 15
    ).is_not_black  # 有可驱散的减益效果
    ...
    if matrix.getBadgeCell(47, 14).is_black:
        ...
        status["unitChannelIcon"] = matrix.getBadgeCell(
            47, 14
        ).title  # 正在通道法术的技能
    ...
    status["unitEnemyCount"] = round(
        matrix.getCell(57, 14).decimal * 51
    )  # 近战敌人数量
    status["unitIsSpellTargeting"] = matrix.getCell(57, 15).is_not_black  # 正在选择目标

    status["unitIsChatInputActive"] = matrix.getCell(
        58, 14
    ).is_not_black  # 正在聊天输入
    status["unitIsInGroupOrRaid"] = matrix.getCell(58, 15).is_not_black  # 在队伍/团队中

    status["unitTrinket1CooldownUsable"] = matrix.getCell(
        59, 14
    ).is_not_black  # 饰品 1可用
    status["unitTrinket2CooldownUsable"] = matrix.getCell(
        59, 15
    ).is_not_black  # 饰品 2可用

    status["unitHealthstoneCooldownUsable"] = matrix.getCell(
        60, 14
    ).is_not_black  # 生命石可用
    status["unitHealingPotionCooldownUsable"] = matrix.getCell(
        60, 15
    ).is_not_black  # 治疗药水可用
...
def get_enemy_status(matrix: MatrixDecoder, x: int, y: int) -> dict[str, Any]:
    ...
    status["unitClass"] = COLOR_MAP["CLASS"].get(
        matrix.getCell(x + 1, y + 0).color_string, "UNKNOWN"
    )  # 职业
    status["unitRole"] = COLOR_MAP["ROLE"].get(
        matrix.getCell(x + 1, y + 1).color_string, "NONE"
    )  # 职责
    ...
    status["unitIsInRangedRange"] = matrix.getCell(
        x + 4, y + 0
    ).is_not_black  # 在远程范围
    status["unitIsInMeleeRange"] = matrix.getCell(
        x + 4, y + 1
    ).is_not_black  # 在近战范围
    ...
    if matrix.getBadgeCell(x + 6, y + 0).is_black:
        ...
        status["unitCastIcon"] = matrix.getBadgeCell(
            x + 6, y + 0
        ).title  # 正在释放的技能
        status["unitCastDuration"] = matrix.getCell(
            x + 10, y + 0
        ).percent  # 施法持续时间
        # status['unitCastIsInterruptible'] = matrix.getCell(x+11, y+0).is_not_black  # 施法是否可中断
        status["unitCastIsInterruptible"] = bool(
            matrix.getCell(x + 11, y + 0).color_string == "255,255,60"
        )  # 施法是否可中断
    ...
    if matrix.getBadgeCell(x + 8, y + 0).is_black:
        ...
        status["unitChannelDuration"] = matrix.getCell(
            x + 10, y + 1
        ).percent  # 通道持续时间
        # status['unitChannelIsInterruptible'] = matrix.getCell(x+11, y+1).is_not_black  # 通道是否可中断
        status["unitChannelIsInterruptible"] = bool(
            matrix.getCell(x + 11, y + 1).color_string == "255,255,60"
        )  # 通道是否可中断
...
def get_party_all(matrix: MatrixDecoder, y: int = 19) -> dict[str, Any]:
    ...
    for i in range(1, 5):
        ...
        if party_exist:
            ...
            result[party_key]["status"]["unitIsAlive"] = matrix.getCell(
                baseX - 9, statusY + 1
            ).is_not_black
            # unitClass\unitRole
            result[party_key]["status"]["unitClass"] = COLOR_MAP["CLASS"].get(
                matrix.getCell(baseX - 8, statusY).color_string, "UNKNOWN"
            )  # 职业
            result[party_key]["status"]["unitRole"] = COLOR_MAP["ROLE"].get(
                matrix.getCell(baseX - 8, statusY + 1).color_string, "NONE"
            )  # 职责
            # unitHealthPercent\unitPowerPercent
            result[party_key]["status"]["unitHealthPercent"] = matrix.getCell(
                baseX - 7, statusY
            ).percent  # 血量百分比
            result[party_key]["status"]["unitPowerPercent"] = matrix.getCell(
                baseX - 7, statusY + 1
            ).percent  # 能量百分比
            # unitIsEnemy /unitCanAttack
            result[party_key]["status"]["unitIsEnemy"] = matrix.getCell(
                baseX - 6, statusY
            ).is_not_black  # 敌人
            result[party_key]["status"]["unitCanAttack"] = matrix.getCell(
                baseX - 6, statusY + 1
            ).is_not_black  # 可以被攻击
            # unitIsInRangedRange/unitIsInMeleeRange
            result[party_key]["status"]["unitIsInRangedRange"] = matrix.getCell(
                baseX - 5, statusY
            ).is_not_black  # 在远程范围
            result[party_key]["status"]["unitIsInMeleeRange"] = matrix.getCell(
                baseX - 5, statusY + 1
            ).is_not_black  # 在近战范围
            # unitIsInCombat/unitIsTarget
            result[party_key]["status"]["unitIsInCombat"] = matrix.getCell(
                baseX - 4, statusY
            ).is_not_black  # 在战斗中
            result[party_key]["status"]["unitIsTarget"] = matrix.getCell(
                baseX - 4, statusY + 1
            ).is_not_black  # 是当前目标
            # unitHasBigDefense/unitHasDispellableDebuff
            result[party_key]["status"]["unitHasBigDefense"] = matrix.getCell(
                baseX - 3, statusY
            ).is_not_black  # 有大防御
            result[party_key]["status"]["unitHasDispellableDebuff"] = matrix.getCell(
                baseX - 3, statusY + 1
            ).is_not_black  # 有可移除的法术
            result[party_key]["status"]["isPlayerCastingTarget"] = matrix.getCell(
                baseX - 2, statusY
            ).is_not_black  # 玩家正在被施法选中
            # DejaVu\\05_slots\\52_party_bar.lua
            result[party_key]["status"]["damage_absorbs"] = (
                matrix.readBarValue(baseX - 20, statusY, 10) / 2
            )  # 伤害吸收 # 注意，吸收盾的条最大现在是血量的一半，所以这里除以2映射到百分比。
            result[party_key]["status"]["heal_absorbs"] = (
                matrix.readBarValue(baseX - 20, statusY + 1, 10) / 2
            )  # 治疗吸收# 注意，吸收盾的条最大现在是血量的一半，所以这里除以2映射到百分比。
            # DejaVu\\05_slots\\51_party_aura.lua
            result[party_key]["buff"] = matrix.readAura(
                x=baseX - 20, y=y, length=7
            )  # buff
            result[party_key]["debuff"] = matrix.readAura(
                x=baseX - 6, y=y, length=3
            )  # debuff
    return result
...
def extract_all_data(matrix: MatrixDecoder) -> dict[str, Any]:
    data: dict[str, Any] = {
        ...
        "target": {"unitToken": "target", "exists": False, "debuff": [], "status": {}},
        "focus": {"unitToken": "focus", "exists": False, "debuff": [], "status": {}},
        "mouseover": {
            "unitToken": "mouseover",
            "exists": False,
            "debuff": [],
        ...
        "spell_stop_list": matrix.readBadgeCellList(43, 26, 10),  # 停止施法的法术
        "spell_queue_window": matrix.getCell(57, 9).mean
        / 100,  # 映射到秒，游戏内的毫秒/10。
        "burst_time": matrix.getCell(82, 0).decimal * 60,
...
if __name__ == "__main__":
    ...
    screenshot_array = np.array(
        Image.open(r"terminal/pixelcalc/monitor_1_small.png").convert("RGB")
    )
