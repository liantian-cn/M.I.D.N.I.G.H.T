...
class DemonHunterDevourer(BaseRotation):
    ...
    def main_rotation(self, ctx: Context) -> tuple[str, float, str]:
        ...
        if aoe_enemy_count_cell is None:
            aoe_enemy_count = 4  # 默认值，10级别
        else:
            aoe_enemy_count = round(aoe_enemy_count_cell.mean / 10)
        ...
        if (
            ...
            # 1. 优先检查“收割”
            if (
                ctx.spell_cooldown_ready("收割", spell_queue_window)
                and not collapsing_star_exists
            ):
        ...
        # 爆发时在没有噬欲时刻时虚空射线好了就用
        if (
            collapsing_star_exists  # 爆发状态开关
            and not moment_of_craving_exists  # 没有噬欲时刻 (核心需求)
            and not player.isMoving  # 没在移动 (射线通常需要引导或读条)
            and ctx.spell_cooldown_ready("虚空射线", spell_queue_window)
        ):
            if target.isInRangedRange:
                return self.cast("虚空射线")
