...
-- 插件级变量定义/引用
local PARTY_HEAL_ABSORB = addonTable.CELL_CLASSIFICATION.PARTY_HEAL_ABSORB
...
描述：
状态条直接接收可能为 secret 的治疗吸收量，阈值保持 10000；成员缺席时仅将本模块的对应条归零。

主要变量信息：
ABSORB_THRESHOLD 是原有治疗吸收阈值；absorbBars 保存四个固定 party 行的独立状态条。

修改记录：
2026-07-26：按小队属性拆分需求迁移治疗吸收状态。
]]
...
local function InitFrame()
    ...
    for memberIndex, unitToken in ipairs(PARTY_UNITS) do
        ...
        local red = PARTY_HEAL_ABSORB / 255
        local green = memberIndex / 255
        local y = PARTY_ROW_Y[memberIndex]

        absorbBar:SetSize(size.CELL, size.CELL)
        absorbBar:SetPoint("TOPLEFT", parent, "TOPLEFT", 6 * size.CELL, -(y - 1) * size.CELL)
    ...
    local function updateBars()
        for memberIndex, unitToken in ipairs(PARTY_UNITS) do
            if UnitExists(unitToken) then
                absorbBars[memberIndex]:SetValue(UnitGetTotalHealAbsorbs(unitToken))
            else
    ...
    eventFrame:RegisterUnitEvent("UNIT_HEAL_ABSORB_AMOUNT_CHANGED", "party1", "party2", "party3", "party4")
